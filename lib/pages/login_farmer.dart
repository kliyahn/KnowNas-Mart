// lib/pages/login_farmer.dart
// ignore_for_file: deprecated_member_use
import 'package:flutter/material.dart';
import 'package:supabase_flutter/supabase_flutter.dart';
import '../utils/supabase_service.dart';
import '/pages/farmer_dashboard.dart';
import '/pages/createfarmer_account.dart';

class LoginFarmerPage extends StatefulWidget {
  const LoginFarmerPage({super.key});

  @override
  State<LoginFarmerPage> createState() => _LoginFarmerPageState();
}
    ...
    try {
      ...
      if (response.user != null) {
        ...
        if (role == 'Farmer') {
          ...
          // Deny login if they are a Buyer trying to use the Farmer login
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Login denied: Your account is registered as a Buyer/Wholesaler. Please use the correct login page.')),
          );
            ...
            child: Padding(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('Log In Farmer', style: TextStyle(fontSize: 18, color: Colors.black54, fontWeight: FontWeight.w500)),
                    ...
                    Center(
                      child: Column(
                        children: const [
                          ...
                          SizedBox(height: 8),
                          Text('Login as Farmer', style: TextStyle(fontSize: 16, color: Colors.black54)), 
                        ],
                      ),
                    ),
                    ...
                    Row(
                      ...
                      children: [
                        _buildSocialButton(onTap: () {print('Google login');}, child: Image.asset('assets/logo_google.png', width: 30, height: 30)),
                        const SizedBox(width: 20),
                        _buildSocialButton(onTap: () {print('Apple login');}, child: const Icon(Icons.apple, size: 35, color: Colors.black)),
                        const SizedBox(width: 20),
                        _buildSocialButton(onTap: () {print('Facebook login');}, child: Image.asset('assets/logo_facebook.png', width: 30, height: 30)),
                      ],
                    ),
                    const SizedBox(height: 40),
                    // Create Account
                    Center(
                      child: GestureDetector(
                        onTap: () {
                          // Ensure this navigates to the Farmer Sign Up page
                          Navigator.push(context, MaterialPageRoute(builder: (context) => const CreateFarmerAccountPage()));
                        },
                        child: RichText(
                          text: const TextSpan(
