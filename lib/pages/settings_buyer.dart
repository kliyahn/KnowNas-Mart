// lib/pages/settings_buyer.dart
import 'package:flutter/material.dart';
import '../utils/supabase_service.dart';
import '../widgets/bottom_nav_buyer.dart';
import 'login_buyer.dart';

class SettingsBuyerPage extends StatefulWidget {
  const SettingsBuyerPage({super.key});
  @override
  State<SettingsBuyerPage> createState() => _SettingsBuyerPageState();
}

class _SettingsBuyerPageState extends State<SettingsBuyerPage> {
  final SupabaseService _supabaseService = SupabaseService();
  String _fullName = 'Loading...';
  String _email = 'Loading...';
  String _phone = 'N/A';
  String _location = 'N/A'; // Stores Buyer Address (from farm_location column)
  String _role = 'Wholesaler/Buyer';
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _role = response['role'] ?? 'Wholesaler/Buyer';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildInfoRow('Email', _email), // DYNAMIC EMAIL
                        const SizedBox(height: 10),
                        _buildInfoRow('Phone', _phone), // DYNAMIC PHONE
                        const SizedBox(height: 10),
                        _buildInfoRow('Address', _location), // DYNAMIC LOCATION/ADDRESS
                      ],
                    ),
                  ),
                ],
      ...
      bottomNavigationBar: const BottomNavBarBuyer(currentIndex: 4),
    );
  }
  ...
  Widget _buildSettingsTile(BuildContext context, {required IconData icon, required String title, required VoidCallback onTap}) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(icon, color: const Color(0xFF2D5F3F), size: 24),
            const SizedBox(width: 15),
            Expanded(
              child: Text(title, style: const TextStyle(color: Color(0xFF2D5F3F), fontSize: 16, fontWeight: FontWeight.w600)),
            ),
            const Icon(Icons.chevron_right, color: Color(0xFF2D5F3F), size: 24),
          ],
        ),
      ),
    );
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: const Text('Log Out', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2D5F3F))),
          content: const Text('Are you sure you want to log out?', style: TextStyle(fontSize: 16)),
          actions: [
            TextButton(onPressed: () { Navigator.of(context).pop(); }, child: const Text('Cancel', style: TextStyle(color: Colors.grey, fontSize: 16))),
            ElevatedButton(
              onPressed: _handleLogout,
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF2D5F3F), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
